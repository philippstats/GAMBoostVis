...
ploter <- function(object, select=NULL,at.step=NULL, print = TRUE) {
  ...
  for (i in select) { #' 
    ...
    ggg <- ggplot() + #environment = environment()
  ...
  return(ggg) #' FIX_ME_PLEASE: If several plots are created (which is default) only the last plot is returned
}